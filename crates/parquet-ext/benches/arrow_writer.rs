...
/// Arrow's sequential column encoding (baseline).
fn encode_arrow_sequential(
    bench: &mut Bencher,
    batch: &RecordBatch,
...
/// parquet-ext's parallel column encoding.
fn encode_parquet_ext_parallel(
    bench: &mut Bencher,
    batch: &RecordBatch,
    props: Option<WriterProperties>,
    num_workers: usize,
    ...
    let parquet_schema = ArrowSchemaConverter::new()
        ...
        .convert(batch.schema_ref())?;
    let writer = SerializedFileWriter::new(&mut file, parquet_schema.root_schema_ptr(), props)?;
    let mut encoder_factory = EncoderFactory::new(&writer, &batch.schema());

    bench.iter(|| {
        let encoder = encoder_factory.try_next_encoder().unwrap();
        let encoded = encoder.encode(&[black_box(batch).clone()], None, num_workers);
        black_box(encoded).unwrap();
    });
...
fn bench_all_writers(c: &mut Criterion) {
    ...
    for (batch_name, batch) in &batches {
        ...
        for (prop_name, prop) in &props {
            group.bench_function(BenchmarkId::new("arrow", *prop_name), |b| {
                encode_arrow_sequential(b, batch, Some(prop.clone())).unwrap()
            });

            group.bench_function(BenchmarkId::new("parquet_ext", *prop_name), |b| {
                encode_parquet_ext_parallel(b, batch, Some(prop.clone()), num_workers).unwrap()
            });
        }
        group.finish();
    }
