...
impl TestCtx {
    ...
    /// Lists all revisions, optionally filtered by active status.
    async fn list_revisions(&self, active: Option<bool>) -> Result<Vec<RevisionInfo>, ListError> {
        self.ampctl_client.revisions().list(active).await
    }
