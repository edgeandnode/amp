...
### 2. Shot Verification Circuit (`shot.circom`)
...
- **Proof Verification**: Integrates Groth16 verifiers for board and shot proofs
...
- `shot.test.ts`: Shot verification circuit tests
...
pnpm run compile:shot     # Compile shot circuit with circom
...
pnpm run setup:shot       # Generate initial proving key  
...
pnpm run contribute:shot  # Add randomness to ceremony
...
pnpm run export:shot      # Generate Solidity verifier
...
./build-circuits.sh shot  # Build only shot circuit
```
...
- Sequential circuit compilation (board and shot)
...
3. **Commitment Testing**: Verify commitment chain integrity in shot circuits
