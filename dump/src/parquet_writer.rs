...
pub async fn commit_metadata(
    ...
    let file_name = location
        .filename()
        .ok_or(format!(
            "ObjectMeta location does not contain a valid file name {location}"
        ))?
        .to_string();
    metadata_db
        .insert_metadata(
...
impl RawTableWriter {
    ...
    ) -> Result<Option<(MetadataBytes, ObjectMeta)>, BoxError> {
    ...
    async fn close(mut self) -> Result<Option<(MetadataBytes, ObjectMeta)>, BoxError> {
    ...
    async fn close_current_file(&mut self) -> Result<(MetadataBytes, ObjectMeta), BoxError> {
...
impl ParquetFileWriter {
    ...
    pub async fn close(
        mut self,
        range: BlockRange,
    ) -> Result<(MetadataBytes, ObjectMeta), BoxError> {
        ...
        let (_, metadata_bytes) =
            read_metadata_bytes_from_parquet(&object_meta, object_store).await?;

        Ok((metadata_bytes, object_meta))
    }
