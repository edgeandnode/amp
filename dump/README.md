...
```
will produce `transfers`, `mints`, and `burns` parquet files, where each column matches the field of the corresponding event message. All non-repeated fields in the module output are dropped from the schema.
