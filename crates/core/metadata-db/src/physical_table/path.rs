//! Physical table path new-type wrapper for database values
//!
//! This module provides a [`Path`] new-type wrapper around [`Cow<str>`] that maintains
//! physical table path invariants for database operations. The type provides efficient handling
...
//! - Path format: `<dataset_name>/<table_name>/<revision_uuid>`
//! - Path must be a valid relative path (no leading slash)
//! - Path components must follow dataset/table/UUID naming conventions

use std::borrow::Cow;

/// An owned physical table path type for database return values and owned storage scenarios.
///
/// This is a type alias for `Path<'static>`, specifically intended for use as a return type from
/// database queries or in any context where a physical table path with owned storage is required.
/// Prefer this alias when working with paths that need to be stored or returned from the database,
/// rather than just representing a path with owned storage in general.
pub type PathOwned = Path<'static>;

/// A physical table path wrapper for database values.
///
/// This new-type wrapper around `Cow<str>` maintains physical table path invariants for database
...
impl<'a> Path<'a> {
    ...
    ///
    /// # Safety
    /// The caller must ensure the provided path upholds the physical table path invariants.
