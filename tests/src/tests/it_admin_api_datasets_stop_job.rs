...
async fn stop_failed_job_is_idempotent() {
    ...
    //* Then
    assert_eq!(
        job_info_before.status, "FAILED_RECOVERABLE",
        "job should be in FAILED_RECOVERABLE state, got: {}",
        job_info_before.status
    );
    assert!(
        result.is_ok(),
    ...
    assert_eq!(
        job_info_after.status, "FAILED_RECOVERABLE",
        "job should remain in FAILED_RECOVERABLE state after stop attempt"
    );
}
