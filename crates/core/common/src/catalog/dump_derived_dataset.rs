//! Derived dataset catalog types and catalog creation functions.
//!
//! This module provides catalog creation specifically for derived dataset execution.
//! It works with pre-resolved dataset dependencies (DepAlias → Hash mappings) for
//! deterministic, reproducible derived dataset dumps.
...
///
/// This function builds a complete catalog containing:
/// - **Physical tables**: Table metadata with parquet file locations from the data store
/// - **Logical catalog**: Table schemas and UDFs required for SQL query planning
///
/// The catalog enables deterministic, reproducible derived dataset execution by using
/// pre-resolved dataset dependencies (DepAlias → Hash mappings) from the dataset manifest.
///
/// ## Where Used
///
...
///
/// Unlike validation functions that work with unresolved dependencies:
/// - All dependencies MUST be pre-resolved to `HashReference` (content-addressable hashes)
/// - Uses locked dataset hashes from the manifest for deterministic execution
/// - No dynamic resolution or "latest" version lookups occur
/// - Ensures reproducible dumps across different executions
///
/// ## Implementation
///
/// The function:
/// 1. Calls [`create_logical`] to resolve datasets and build the logical catalog
...
pub async fn create_phy(
    ...
    .await
    .map_err(CreateCatalogError::CreateLogical)?;
...
/// Errors that can occur when creating a derived dataset catalog.
///
/// Returned by [`create_phy`] when catalog creation fails.
