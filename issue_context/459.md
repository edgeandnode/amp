...
## How it was fixed
Systematically renamed `input_batch_size_blocks` to `microbatch_max_interval` throughout codebase including CLI parameters, function signatures, variable names, imports, and documentation. All occurrences in 9 files updated to use consistent naming.

# 4. Move to config file

## Objective
Remove CLI parameter and enable global configuration of microbatch intervals.

## How it was fixed
Moved `microbatch_max_interval` from CLI parameter to main config file. Added field to `Config` and `ConfigFile` structs with default value of 100,000. Removed CLI parameter while maintaining test override capability through optional parameter in test helper functions. Updated test configuration to include the new setting.
