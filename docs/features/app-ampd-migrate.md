...
The
`ampd migrate` command runs database migrations on the metadata database. It connects to the configured PostgreSQL instance, applies any pending schema migrations, and exits. This is a one-shot administrative command used during upgrades or initial setup.
...
- **Metadata Database
  **: PostgreSQL database storing job state, worker registrations, dataset definitions, and table revisions
...
`auto_migrate = true` in the config file. When enabled, services automatically apply migrations on startup, making the explicit
