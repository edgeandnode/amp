...
use prost_reflect::{DescriptorPool, Kind, MessageDescriptor, Cardinality};
...
impl Tables {
    ...
    pub fn from_package(package: Package, output_module: String) -> Result<Self, anyhow::Error> {
        ...
        let module = package.modules.as_ref().unwrap().modules.iter().find(|&m| m.name == output_module).context(format!("module {} not found", output_module))?;
        let output_type = Self::parse_message_type(module.output.as_ref().unwrap().r#type.as_str());
        let pool = DescriptorPool::decode(descr_set.encode_to_vec().as_slice()).context("failed to decode descriptor pool from package")?;
        let message_descriptor = pool.get_message_by_name(output_type.as_str()).unwrap();
        let tables = message_descriptor.fields().filter_map(|field| {
            ...
            fields.extend(message.fields().map(|f| {
