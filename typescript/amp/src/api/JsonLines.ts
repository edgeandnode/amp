 ...
 */
export class JsonLines extends Context.Tag("Amp/JsonLines")<
  JsonLines,
  {
    /**
    ...
    readonly stream: {
      <A, I, R>(
        ...
        A,
        HttpClientError.HttpClientError | JsonLinesError,
        R
      >
      <A, I, R>(
        schema: Schema.Schema<A, I, R>,
...
export const layer = (url: string, options?: JsonLinesOptions) =>
  make(url, options).pipe(
    Layer.effect(JsonLines),
