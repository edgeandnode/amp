...
/// Update when adding multi-version support or upgrading the minimum
/// supported PostgreSQL version.
///
/// Tested with PostgreSQL 16, 17, 18. Prefer PostgreSQL 18 (per issue #1383).
#[allow(dead_code)] // Used in rustdoc references, not runtime code
...
impl PostgresBuilder {
    ...
    ///
    /// The returned future uses `tokio::select!` to monitor three events:
    /// - **Stop request**: the [`Handle`](super::service::Handle) signals shutdown
    /// - **Shutdown signal**: SIGINT (Ctrl+C) or SIGTERM from the OS
    /// - **Unexpected child exit**: the postgres process crashes or exits on its own
    ///
    /// On stop request or signal, the future performs graceful shutdown
    /// (SIGTERM → wait → SIGKILL). On unexpected exit, it returns an error.
    ///
    /// # Errors
    ///
    /// Returns `PostgresError` if:
    /// - Required PostgreSQL binaries are not found
    /// - Data directory cannot be created or initialized
    /// - The server fails to start or become ready
    pub async fn start(
        self,
    ) -> Result<
        (
    ...
    > {
        ...
        let handle = super::service::Handle::new(url, data_dir, tx_stop);

        let fut = async move {
            // Use an enum to track why we're shutting down, avoiding
            // moving `process` into multiple select! branches.
            enum ShutdownReason {
                StopRequested,
                Signal,
                ProcessExited(Result<std::process::ExitStatus, std::io::Error>),
            }

            // Pin the process so we can take &mut references in select!
            let mut process = process;

            let reason = tokio::select! {
                _ = rx_stop.recv() => ShutdownReason::StopRequested,
                _ = super::service::shutdown_signal() => ShutdownReason::Signal,
            ...
            match reason {
                ShutdownReason::ProcessExited(status) => {
                    ...
                    Err(PostgresError::UnexpectedExit {
    ...
    pub(crate) async fn start_process(self) -> Result<PostgresProcess, PostgresError> {
        ...
        // Spawn async tasks to forward subprocess output to tracing (IC-6).
    ...
    ///
    /// The child process is spawned with `kill_on_drop(true)` as a SIGKILL
    /// safety net, and both stdout/stderr are piped for tracing (IC-6).
    ///
    /// ## PostgreSQL Reference
    ///
    ...
    async fn start_postgres_server(&self) -> Result<Child, PostgresError> {
        ...
        // Safety net: kill child on handle drop (sends SIGKILL if Drop impl's
        // SIGTERM doesn't complete in time)
        cmd.kill_on_drop(true);

        // Pipe both stdout and stderr so we can forward to tracing (IC-6)
        let child = cmd
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
...
impl PostgresProcess {
    ...
    async fn shutdown_inner(&mut self) -> Result<(), PostgresError> {
        ...
        // Send SIGTERM for graceful shutdown via nix crate (safe Rust, no shell)
        #[cfg(unix)]
        if let Some(pid) = self.child.id() {
            let nix_pid = nix::unistd::Pid::from_raw(pid as i32);
            if let Err(err) = nix::sys::signal::kill(nix_pid, nix::sys::signal::Signal::SIGTERM) {
                tracing::warn!(pid = pid, error = %err, "Failed to send SIGTERM to PostgreSQL");
            }
        }
...
impl Drop for PostgresProcess {
    fn drop(&mut self) {
        ...
        // giving postgres a chance to flush WAL before kill_on_drop(true) sends
        // SIGKILL as the safety net.
        #[cfg(unix)]
        if let Some(pid) = self.child.id() {
        ...
        // kill_on_drop(true) on the Command sends SIGKILL as final safety net
    }
}
