    ...
    match command {
        Command::Solo {
            ...
            if !flight_server && matches!(env::var("FLIGHT_SERVER").as_deref(), Ok("true" | "1")) {
                eprintln!(
                    "warning: env var FLIGHT_SERVER is deprecated, use AMP_FLIGHT_SERVER instead"
                );
                flight_server = true;
            }
            ...
            if !jsonl_server && matches!(env::var("JSONL_SERVER").as_deref(), Ok("true" | "1")) {
                eprintln!(
                    "warning: env var JSONL_SERVER is deprecated, use AMP_JSONL_SERVER instead"
                );
                jsonl_server = true;
            }
            ...
            if !admin_server && matches!(env::var("ADMIN_SERVER").as_deref(), Ok("true" | "1")) {
                eprintln!(
                    "warning: env var ADMIN_SERVER is deprecated, use AMP_ADMIN_SERVER instead"
                );
                admin_server = true;
            }
