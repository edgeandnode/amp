...
class TestPostgreSQLLoader:
    ...
    @patch('src.nozzle.loaders.implementations.postgresql_loader.csv')
    def test_copy_arrow_batch(self, mock_csv, mock_config, test_batch):
        ...
        # Mock CSV writing
        mock_output = Mock()
        mock_output.getvalue.return_value = b'1\tuser_1\t0.1\tTrue\t2023-01-01 00:00:00\n'

        mock_csv.write_csv = Mock()
        mock_csv.WriteOptions = Mock()

        # Mock StringIO behavior
        with patch('src.nozzle.loaders.implementations.postgresql_loader.io.BytesIO', return_value=mock_output):
            with patch('src.nozzle.loaders.implementations.postgresql_loader.io.StringIO') as mock_string_io:
                mock_buffer = Mock()
                mock_string_io.return_value = mock_buffer
        ...
        assert call_args[1]['columns'] ==  ['id', 'name', 'value', 'active', 'created_at']

    @patch('src.nozzle.loaders.implementations.postgresql_loader.csv')
    def test_copy_arrow_batch_overwrite_mode(self, mock_csv, mock_config, test_batch):
    ...
    @patch('src.nozzle.loaders.implementations.postgresql_loader.csv')
    def test_load_batch_success(self, mock_csv, mock_config, test_batch, mock_pool_setup):
        ...
        # Mock IO operations
        with patch('src.nozzle.loaders.implementations.postgresql_loader.io.BytesIO'):
            with patch('src.nozzle.loaders.implementations.postgresql_loader.io.StringIO'):
...
class TestPostgreSQLLoaderEdgeCases:
    ...
    def test_null_value_handling(self, loader):
        ...
        data = [
            ...
            pa.array(['a', None, 'c', 'd'], type=pa.string())
        ]
        schema = pa.schema([
            pa.field('id', pa.int64()),
            pa.field('name', pa.string())
        ...
        # Mock CSV operations
        with patch('src.nozzle.loaders.implementations.postgresql_loader.csv') as mock_csv:
            with patch('src.nozzle.loaders.implementations.postgresql_loader.io.BytesIO') as mock_bio:
                with patch('src.nozzle.loaders.implementations.postgresql_loader.io.StringIO') as mock_sio:
                    mock_output = Mock()
                    mock_output.getvalue.return_value = b'1\ta\n2\t\\N\n\\N\tc\n4\td\n'
                    mock_bio.return_value = mock_output

                    mock_buffer = Mock()
                    mock_sio.return_value = mock_buffer
    ...
    def test_copy_operation_failure(self, loader):
        ...
        # Mock IO operations
        with patch('src.nozzle.loaders.implementations.postgresql_loader.csv'):
            with patch('src.nozzle.loaders.implementations.postgresql_loader.io.BytesIO'):
                with patch('src.nozzle.loaders.implementations.postgresql_loader.io.StringIO'):
