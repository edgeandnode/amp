...
//!
//! This module provides a [`VersionHash`] new-type wrapper around [`Cow<str>`] that maintains
...
//! at system boundaries through types like [`datasets_common::VersionHash`], which enforce the
...
//! database data."
//!
//! Types that convert into [`VersionHash`] are responsible for ensuring invariants are met:
...
///
/// This is a type alias for `VersionHash<'static>`, specifically intended for use as a return type from
...
pub type VersionHashOwned = VersionHash<'static>;
...
pub struct VersionHash<'a>(Cow<'a, str>);

impl<'a> VersionHash<'a> {
    /// Create a new VersionHash wrapper from a reference to str (borrowed)
    ///
    /// # Safety
    ...
    /// Create a new VersionHash wrapper from an owned String
    ///
    /// # Safety
    /// The caller must ensure the provided version hash upholds the version hash invariants.
    /// This method does not perform validation. Failure to uphold the invariants may
    /// cause undefined behavior.
    pub fn from_owned_unchecked(hash: String) -> VersionHash<'static> {
        VersionHash(Cow::Owned(hash))
    }

    /// Consume and return the inner String (owned)
    pub fn into_inner(self) -> String {
        match self {
            VersionHash(Cow::Owned(hash)) => hash,
            VersionHash(Cow::Borrowed(hash)) => hash.to_owned(),
        }
    }
...
impl<'a> std::ops::Deref for VersionHash<'a> {
    type Target = str;
...
impl<'a> AsRef<str> for VersionHash<'a> {
    fn as_ref(&self) -> &str {
        &self.0
    }
}

impl<'a> PartialEq<&str> for VersionHash<'a> {
    fn eq(&self, other: &&str) -> bool {
        self.as_str() == *other
    }
}

impl<'a> PartialEq<VersionHash<'a>> for &str {
    fn eq(&self, other: &VersionHash<'a>) -> bool {
        *self == other.as_str()
    }
}
...
impl<'a> PartialEq<String> for VersionHash<'a> {
    fn eq(&self, other: &String) -> bool {
        self.as_str() == other
    }
}

impl<'a> PartialEq<VersionHash<'a>> for String {
    fn eq(&self, other: &VersionHash<'a>) -> bool {
        self == other.as_str()
    }
}

impl<'a> std::fmt::Display for VersionHash<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.0.fmt(f)
    }
}

impl<'a> std::fmt::Debug for VersionHash<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.0.fmt(f)
    }
}

impl sqlx::Type<sqlx::Postgres> for VersionHash<'_> {
    fn type_info() -> sqlx::postgres::PgTypeInfo {
        <String as sqlx::Type<sqlx::Postgres>>::type_info()
    }
}

impl<'a> sqlx::Encode<'_, sqlx::Postgres> for VersionHash<'a> {
    fn encode_by_ref(
        &self,
...
impl<'r> sqlx::Decode<'r, sqlx::Postgres> for VersionHashOwned {
    fn decode(value: sqlx::postgres::PgValueRef<'r>) -> Result<Self, sqlx::error::BoxDynError> {
        ...
        Ok(VersionHash::from_owned_unchecked(s))
    }
}
