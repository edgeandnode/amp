...
async fn init_creates_directory_structure() -> Result<()> {
    ...
    assert!(
        temp.nozzleup_binary().exists(),
        "nozzleup binary not created"
    );
...
async fn list_shows_no_versions_when_empty() -> Result<()> {
    ...
    nozzleup::commands::list::run(Some(temp.path().to_path_buf()))?;
...
async fn use_switches_to_installed_version() -> Result<()> {
    ...
    // Switch to v1.0.0
    nozzleup::commands::use_version::run(
        Some(temp.path().to_path_buf()),
...
async fn use_fails_for_non_existent_version() -> Result<()> {
    ...
    let result = nozzleup::commands::use_version::run(
        Some(temp.path().to_path_buf()),
...
async fn uninstall_removes_version() -> Result<()> {
    ...
    nozzleup::commands::uninstall::run(Some(temp.path().to_path_buf()), "v1.0.0")?;
...
async fn uninstall_fails_for_non_existent_version() -> Result<()> {
    ...
    let result = nozzleup::commands::uninstall::run(Some(temp.path().to_path_buf()), "v99.99.99");
...
async fn build_from_local_path_with_custom_name() -> Result<()> {
    ...
    // Create a temporary directory that looks like a nozzle repo
    let fake_repo = TempDir::new()?;
    let target_dir = fake_repo.path().join("target/release");
    fs::create_dir_all(&target_dir)?;

    // Create a mock nozzle binary
    let mock_binary = target_dir.join("nozzle");
    fs::write(&mock_binary, "#!/bin/sh\necho 'nozzle test-version'")?;
    ...
    let result = nozzleup::commands::build::run(
        Some(temp.path().to_path_buf()),
