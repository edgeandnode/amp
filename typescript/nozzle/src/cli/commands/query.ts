...
export const query = Command.make("query", {
  ...
  const table = yield* flight.table(args.query).pipe(Effect.map((table) => Option.match(args.limit, {
  ...
  yield* Match.value(args.format).pipe(
    Match.when("json", () => Effect.succeed([...table]).pipe(
      Effect.flatMap(Schema.encodeUnknown(Schema.Array(schema))),
      Effect.map((_) => JSON.stringify(_, null, 2)),
      Effect.flatMap(Console.log),
    )),
    Match.when("table", () => Effect.succeed([...table]).pipe(  
      Effect.flatMap(Schema.encodeUnknown(Schema.Array(schema))),
      Effect.flatMap(Console.table),
    )),
    Match.when("pretty", () => Stream.fromIterable([...table]).pipe(
