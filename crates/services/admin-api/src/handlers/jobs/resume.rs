...
///
/// Resumes a stopped job using the specified job ID. This is an idempotent
/// operation that handles job termination requests safely.
///
/// ## Path Parameters
/// - `id`: The unique identifier of the job to resume (must be a valid JobId)
///
/// ## Response
/// - **200 OK**: Job resume request processed successfully, or job already in scheduled or running state (idempotent)
...
/// - `RESUME_JOB_ERROR`: Database error during resume operation execution
/// - `UNEXPECTED_STATE_CONFLICT`: Internal state machine error (indicates a bug)
///
/// ## Idempotent Behavior
/// This handler is idempotent - resuming a job that's already in a scheduled or running state returns success (200).
/// This allows clients to safely retry stop requests without worrying about conflict errors.
///
/// The desired outcome of a resume request is that the job is running. If the job is already
/// stopped, completed, or failed, this outcome is achieved, so we return success.
///
/// ## Behavior
/// This handler provides idempotent job resuming with the following characteristics:
/// - Jobs already in scheduled or running states return success (idempotent)
/// - Only stopped jobs transition to scheduled state
/// - Job lookup and resume request are performed atomically within a single transaction
/// - Database layer validates state transitions and prevents race conditions
///
/// ## State Transitions
/// Valid resume transitions:
/// - Stopped → Scheduled (200 OK)
///
/// Already scheduled or running (idempotent - return success):
/// - Scheduled → no change (200 OK)
/// - Running → no change (200 OK)
///
/// The handler:
/// - Validates and extracts the job ID from the URL path
/// - Delegates to scheduler for atomic resume operation (job lookup + resume + worker notification)
/// - Returns success if job is already in scheduled or running state (idempotent)
...
#[cfg_attr(
    ...
    utoipa::path(
        ...
        path = "/jobs/{id}/resume",
        tag = "jobs",
        operation_id = "jobs_resume",
        params(
            ("id" = i64, Path, description = "Job ID")
        ),
        responses(
            (status = 200, description = "Job resume request processed successfully, or job already in scheduled or running state (idempotent)"),
...
pub async fn handler(
    ...
    // Attempt to resume the job - this operation is atomic and includes job lookup
    if let Err(err) = ctx.scheduler.resume_job(id).await {
        return match err {
            scheduler::ResumeJobError::JobNotFound => {
                tracing::debug!(job_id=%id, "Job not found");
                Err(Error::NotFound { id }.into())
            }
            scheduler::ResumeJobError::JobAlreadyRunning { status } => {
                // Idempotent behavior: job is already in scheduled or running state
                tracing::debug!(job_id=%id, status=%status, "Job already in scheduled or running state, returning success (idempotent)");
                Ok(StatusCode::OK)
            }
            scheduler::ResumeJobError::StateConflict { current_status } => {
                // Unexpected state conflict - this shouldn't happen with current state machine
                tracing::error!(
                    job_id=%id,
                    current_status=%current_status,
                    "Unexpected state conflict during resume operation"
                );
            ...
            scheduler::ResumeJobError::BeginTransaction(_)
            | scheduler::ResumeJobError::GetJob(_)
            | scheduler::ResumeJobError::UpdateJobStatus(_)
            | scheduler::ResumeJobError::CommitTransaction(_)
            | scheduler::ResumeJobError::SendNotification(_) => {
                tracing::error!(error = %err, error_source = logging::error_source(&err), job_id=%id, "Database error during resume operation");
                Err(Error::ResumeJob { id, source: err }.into())
            }
        };
    }

    tracing::info!(job_id=%id, "Job resume request processed successfully");
    Ok(StatusCode::OK)
}
...
pub enum Error {
    ...
    /// Database error during resume operation
    ///
    /// This error occurs when the scheduler encounters a database error
    /// while executing the atomic resume operation (job lookup, state transition, or worker notification).
    ///
    /// This occurs when:
    ...
    #[error("failed to resume job '{id}'")]
    ResumeJob {
        id: JobId,
        source: scheduler::ResumeJobError,
    },
...
impl IntoErrorResponse for Error {
    fn error_code(&self) -> &'static str {
        match self {
            ...
            Error::ResumeJob { .. } => "RESUME_JOB_ERROR",
    ...
    fn status_code(&self) -> StatusCode {
        match self {
            ...
            Error::ResumeJob { .. } => StatusCode::INTERNAL_SERVER_ERROR,
