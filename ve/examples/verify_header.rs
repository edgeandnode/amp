//! Verify internal consistency of headers in a single block.
//! This only handles the validty of the Header data alone,
//! not the whole data of the block
//!
//! Requirements
...
async fn main() -> Result<()> {
    ...
    // Adjust dataset name / namespace to match your AMPD setup.
    let sql = format!(
        r#"
        SELECT
            block_num,
    ...
    println!(
        "Verified header hash for {} blocks in range [{}..={}]",
        verified, first, last
    );
