...
# Diff staging amp-server
[group: 'kubernetes']
k8s_stg_amp_server_diff:
    @k8s/staging/auth.sh
    kubectl diff -k k8s/staging/amp-server || true

# Apply staging amp-server
[group: 'kubernetes']
k8s_stg_amp_server_apply:
    @k8s/staging/auth.sh
    kubectl apply -k k8s/staging/amp-server || true

# Diff staging amp-controller
[group: 'kubernetes']
k8s_stg_amp_controller_diff:
    @k8s/staging/auth.sh
    kubectl diff -k k8s/staging/amp-controller || true

# Apply staging amp-controller
[group: 'kubernetes']
k8s_stg_amp_controller_apply:
    @k8s/staging/auth.sh
    kubectl apply -k k8s/staging/amp-controller || true

# Diff staging amp-gateway
[group: 'kubernetes']
k8s_stg_amp_gateway_diff:
    @k8s/staging/auth.sh
    kubectl diff -k k8s/staging/amp-gateway || true

# Apply staging amp-gateway
[group: 'kubernetes']
k8s_stg_amp_gateway_apply:
    @k8s/staging/auth.sh
    kubectl apply -k k8s/staging/amp-gateway || true
