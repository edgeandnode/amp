//! Internal SQL operations for file metadata management
//!
//! This module contains the low-level database operations for file metadata.
...
pub fn stream_with_details<'c, E>(
    exe: E,
    location_id: LocationId,
) -> impl Stream<Item = Result<FileMetadataWithDetails, sqlx::Error>> + 'c
where
    E: Executor<'c, Database = Postgres> + 'c,
{
    ...
    sqlx::query_as(query).bind(location_id).fetch(exe)
}

/// Get footer bytes for a file by ID
pub async fn get_footer_bytes_by_id<'c, E>(exe: E, id: FileId) -> Result<FooterBytes, sqlx::Error>
where
    E: Executor<'c, Database = Postgres>,
{
...
pub async fn delete_by_ids<'c, E>(exe: E, file_ids: &[FileId]) -> Result<Vec<FileId>, sqlx::Error>
where
    E: Executor<'c, Database = Postgres>,
{
    let query = "DELETE FROM file_metadata WHERE id = ANY($1) RETURNING id";
