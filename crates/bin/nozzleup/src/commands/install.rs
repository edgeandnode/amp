...
pub async fn run(
    ...
    if version_manager.is_installed(&version) {
        ...
        if current_version.as_deref() == Some(&version) {
            ui::success(format!("Already using version {}", ui::version(&version)));
            return Ok(());
        }

        // Switch to this version
        ui::info(format!("Switching to version {}", ui::version(&version)));
    ...
    ui::info(format!("Installing version {}", ui::version(&version)));
    ...
    ui::detail(format!("Platform: {}, Architecture: {}", platform, arch));
    ...
    ui::success(format!("Installed nozzle {}", ui::version(&version)));
    ui::detail("Run 'nozzle --version' to verify installation");
