...
/// Handler for the `POST /revisions/create` endpoint
///
/// Registers a new table revision from a given path.
///
/// ## Request Body
...
///
/// ## Response
/// - **200 OK**: Successfully registered the table revision
...
/// 2. Registers the table revision path in the metadata database
...
#[cfg_attr(
    ...
    utoipa::path(
        ...
        responses(
            (status = 200, description = "Successfully registered the table revision", body = CreateRevisionResponse),
...
pub async fn handler(
    ...
    tracing::debug!(
        ...
        path = %payload.path,
        "table revision registration received"
    );
